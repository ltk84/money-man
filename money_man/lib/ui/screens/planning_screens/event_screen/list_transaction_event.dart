...
class ListTransactionEvent extends StatefulWidget {
  ...
  const ListTransactionEvent(
      ...
      : super(key: key);
  @override
  State<StatefulWidget> createState() => _ListTransactionEvent();
}

class _ListTransactionEvent extends State<ListTransactionEvent> {
  ...
  @override
  void didUpdateWidget(covariant ListTransactionEvent oldWidget) {
    _transactionList = widget.currentList ?? [];
    _beginDate = widget.beginDate;
    _endDate = widget.endDate;
    _controller = ScrollController();
  ...
  Container buildBottomViewByDate(List<List<MyTransaction>> transListSortByDate,
    ...
    transListSortByDate[xIndex].removeWhere((element) => element.eventID != widget.event.id);

    return (transListSortByDate[xIndex].length != 0) ?Container(
